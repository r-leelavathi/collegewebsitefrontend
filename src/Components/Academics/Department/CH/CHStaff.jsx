...
const CHStaff = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/api/faculties')
      .then(response => {
        // Filter the faculty data to show only Chemical Engineering department
        const chemicalFaculty = response.data.filter(faculty => faculty.facultyDepartment === 'Chemical Engineering');
