...
import './../DepartmentActivity.css'; // Import the CSS file
...
const CHActivity = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/api/departmental-activities')
      .then(response => {
        ...
        const filteredData = response.data.filter(activity => activity.department === 'CH');
  ...
  return (
    <div className="departmental-activities-container">
      <h2 className="departmental-activities-title">Chemical Engineering Department Activities</h2>
