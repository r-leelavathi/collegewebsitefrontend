...
const AdminArtLiteratureClubTable = () => {
  ...
  const fetchEvents = async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/artliterature');
  ...
  const handleSave = async (id) => {
    try {
      await axios.put(`http://localhost:8080/api/artliterature/${id}`, editedData);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8080/api/artliterature/${id}`);
  ...
  return (
    <div className="admin-edit-table-container">
      ...
      <h2>Art, Literature and Cultural Club Events</h2>
