...
const AdminSportsTable = () => {
  ...
  const fetchEvents = async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/sportclub');
      setEvents(response.data);
    } catch (error) {
      console.error('Error fetching Sport Club events:', error);
    }
  };
  ...
  const handleSave = async (id) => {
    try {
      await axios.put(`http://localhost:8080/api/sportclub/${id}`, editedData);
      fetchEvents(); // Refresh data after saving
      setEditing(null); // Exit edit mode
    } catch (error) {
      console.error('Error updating Sport Club event:', error);
    }
  };

  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8080/api/sportclub/${id}`);
      fetchEvents(); // Refresh data after deleting
    } catch (error) {
      console.error('Error deleting Sport Club event:', error);
    }
  };
...
export default AdminSportsTable;
